...
def validate_html(text: str, request_data: dict[str, Any] | None, base_dir: Path) -> list[str]:
    ...
    communication_mode = (_meta(text, "communication-mode") or "").upper()
    ...
    if report_type == "CHECKPOINT":
        ...
        if communication_mode not in {"CUSTOMER_SALES", "BUYER_ADVISORY"}:
            errors.append("communication mode must be CUSTOMER_SALES or BUYER_ADVISORY")
        if "이번 분석을" not in text or "핵심 판단" not in text or not any(label in text for label in ("다음 행동", "권장 행동")):
    ...
    if request_data is not None:
        try:
            ...
            if normalized.get("report_type") != "checkpoint" and str(normalized.get("communication_mode", "")).upper() == "BUYER_ADVISORY":
                decision = normalized.get("decision_context") if isinstance(normalized.get("decision_context"), dict) else {}
                numeric_advice = any(decision.get(key) not in (None, "") for key in ("first_offer_krw", "conditional_ceiling_krw", "hold_above_krw", "break_even_price_krw"))
                if numeric_advice:
                    if not str(decision.get("recommendation_basis", "")).strip():
                        errors.append("numeric buyer advice requires decision_context.recommendation_basis")
                    if not str(decision.get("recommendation_basis_date", "")).strip():
                        errors.append("numeric buyer advice requires decision_context.recommendation_basis_date")
                    if not decision.get("recommendation_source_ids"):
