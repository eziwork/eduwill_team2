...
def validate_intake(data: dict[str, Any], base_dir: Path | None = None) -> tuple[list[str], list[str]]:
    ...
    for field in ("budget_krw", "intended_holding_years", "first_offer_krw", "conditional_ceiling_krw", "hold_above_krw", "break_even_price_krw"):
    ...
    communication_mode = communication.get("mode", "CUSTOMER_SALES")
    numeric_advice = any(decision_context.get(field) not in (None, "") for field in ("first_offer_krw", "conditional_ceiling_krw", "hold_above_krw", "break_even_price_krw"))
    if communication_mode == "BUYER_ADVISORY":
        if data.get("customer", {}).get("role") != "BUY":
            errors.append("BUYER_ADVISORY requires customer.role=BUY")
        if numeric_advice:
            if not _nonempty_string(decision_context.get("recommendation_basis")):
                errors.append("numeric buyer advice requires decision_context.recommendation_basis")
            basis_date = str(decision_context.get("recommendation_basis_date") or "")
            if not re.fullmatch(r"\d{4}-\d{2}-\d{2}", basis_date):
                errors.append("numeric buyer advice requires decision_context.recommendation_basis_date in YYYY-MM-DD")
            source_ids = decision_context.get("recommendation_source_ids")
            if not isinstance(source_ids, list) or not source_ids or not all(_nonempty_string(item) for item in source_ids):
                errors.append("numeric buyer advice requires decision_context.recommendation_source_ids")
