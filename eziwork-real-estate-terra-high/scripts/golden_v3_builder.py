...
def _page_footer(brand: str, page: int, logo_uri: str = "") -> str:
    logo = f'<img class="brand-logo" src="{logo_uri}" alt="{esc(brand)} 로고">' if logo_uri else ""
    return f'<div class="brand">{logo}<span>{esc(brand)}</span></div><div class="page-no">{page:02d} / {PAGE_COUNT:02d}</div>'
...
def _decision_amount(decision: dict[str, Any], key: str) -> str:
    value = number(decision.get(key))
    return fmt_krw(value) if value is not None else "추가 산정 필요"


def _role_copy(role: str, target: str, communication_mode: str, decision: dict[str, Any]) -> dict[str, str]:
    if communication_mode == "BUYER_ADVISORY":
        first_offer = _decision_amount(decision, "first_offer_krw")
        ceiling = _decision_amount(decision, "conditional_ceiling_krw")
        amounts_ready = "추가 산정 필요" not in {first_offer, ceiling}
        return {
            "cover": "가격 근거와 보유·회수 조건, 계약 전 리스크를 함께 검토합니다.",
            "fit": f"{target}의 완료 거래와 현재 호가를 분리해 보고, 미확인 현장·권리 조건을 확인한 뒤 협상 범위를 정해야 합니다.",
            "headline": "확인된 가격 근거와 미확인 조건을 분리해 협상 범위를 정해야 합니다.",
            "cta": f"{first_offer} 제안 → 현장·권리 확인 → {ceiling} 이내 조건부 결정" if amounts_ready else "근거 확인 → 협상안 산정 → 현장·권리 확인 후 결정",
        }
...
def _transaction_history_svg(rows: list[dict[str, Any]], mode: str, target_price: float | None) -> str:
    points = []
    for row in sorted(rows, key=lambda item: str(item.get("contract_date", ""))):
        value = _transaction_value(row, mode)
        date = str(row.get("contract_date") or "")
        if value is not None and date:
            points.append((date, value, str(row.get("building") or ""), number(row.get("floor"))))
    if len(points) < 2:
        return '<div class="visual-empty"><b>계약가격 타임라인 자료 보완</b><span>날짜와 가격이 확인된 완료 거래가 두 건 이상이면 계약 순서별 가격 위치를 표시합니다.</span></div>'
    width, height = 720, 230
    left, right, top, bottom = 62.0, 28.0, 30.0, 48.0
    values = [item[1] for item in points]
    lo = min(values + ([target_price] if target_price is not None else []))
    hi = max(values + ([target_price] if target_price is not None else []))
    pad = max((hi - lo) * .16, 2_000_000)
    lo, hi = lo - pad, hi + pad
    x = lambda index: left + index * (width - left - right) / max(1, len(points) - 1)
    y = lambda value: top + (hi - value) * (height - top - bottom) / max(1, hi - lo)
    grid = []
    for index in range(4):
        value = lo + (hi - lo) * index / 3
        py = y(value)
        grid.append(f'<line x1="{left}" y1="{py:.1f}" x2="{width-right}" y2="{py:.1f}" stroke="#dce7f1"/><text x="{left-8}" y="{py+4:.1f}" text-anchor="end" class="svg-label">{esc(fmt_krw(value))}</text>')
    coords = [(x(index), y(value)) for index, (_date, value, _building, _floor) in enumerate(points)]
    line = " ".join(f"{px:.1f},{py:.1f}" for px, py in coords)
    marks = []
    for index, ((date, value, building, floor), (px, py)) in enumerate(zip(points, coords)):
        floor_text = f" · {int(floor)}층" if floor is not None else ""
        marks.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="6" fill="#0a67ff" stroke="#fff" stroke-width="2"><title>{esc(date)} · {esc(building)}{esc(floor_text)} · {esc(fmt_krw(value))}</title></circle>')
        marks.append(f'<text x="{px:.1f}" y="{py-11:.1f}" text-anchor="middle" class="svg-value">{esc(fmt_krw(value))}</text>')
        marks.append(f'<text x="{px:.1f}" y="{height-18}" text-anchor="middle" class="svg-label">{esc(date[2:7].replace("-", "."))}</text>')
    target = ""
    if target_price is not None:
        ty = y(target_price)
        target = f'<line x1="{left}" y1="{ty:.1f}" x2="{width-right}" y2="{ty:.1f}" stroke="#f37021" stroke-width="2.5" stroke-dasharray="8 5"/><rect x="{width-right-112}" y="{ty-24:.1f}" width="112" height="20" rx="10" fill="#f37021"/><text x="{width-right-56}" y="{ty-10:.1f}" text-anchor="middle" class="svg-target">검토 {esc(fmt_krw(target_price))}</text>'
    return f'<svg viewBox="0 0 {width} {height}" role="img" aria-label="계약일별 실거래 가격 타임라인">{"".join(grid)}{target}<polyline points="{line}" fill="none" stroke="#0a67ff" stroke-width="3" stroke-linejoin="round"/>{"".join(marks)}</svg>'
...
def render_golden_v3(
    ...
    *,
    ...
    brand_logo_uri = image_loader(str(brand.get("logo_path", "")), base_dir)

    def page_footer(brand_value: str, page: int) -> str:
        return _page_footer(brand_value, page, brand_logo_uri)
    ...
    communication_mode = str(data.get("communication_mode") or "CUSTOMER_SALES").upper()
    is_advisory = communication_mode == "BUYER_ADVISORY"
    decision_context = data.get("decision_context") if isinstance(data.get("decision_context"), dict) else {}
    role_copy = _role_copy(str(customer.get("role", "")), target_name, communication_mode, decision_context)
    ...
    listing_visual_spec = listing_section.get("visual", {})
    if not is_advisory and isinstance(listing_visual_spec, dict) and listing_visual_spec.get("type") == "bar":
        label_map = {"표본 최저": "현재 최저", "표본 중앙": "현재 중간", "표본 최고": "현재 최고"}
        listing_visual_spec = dict(listing_visual_spec)
        listing_visual_spec["rows"] = [
            dict(row, label=label_map.get(str(row.get("label", "")), str(row.get("label", ""))), note="확인한 매물")
            for row in listing_visual_spec.get("rows", [])
            if isinstance(row, dict)
        ]
    listing_visual = render_visual(listing_visual_spec, base_dir)
    if not listings:
        listing_visual = '''<div class="visual-empty listing-empty"><span class="empty-kicker">현장 상담 포인트</span><b>최신 선택지는 방문 상담 때 함께 확인하겠습니다</b><p>공개매물은 수시로 바뀌므로, 현재는 실제 계약 사례와 세대의 현장 가치를 중심으로 살펴보시면 됩니다.</p><div class="empty-steps"><span><i>1</i>최신 매물 확인</span><span><i>2</i>조건별 비교</span><span><i>3</i>거래 가능 여부 확인</span></div></div>'''
    ...
    cover_eyebrow = "REAL ESTATE DECISION BRIEF" if is_advisory else "REAL ESTATE CUSTOMER BRIEF"
    cover_report_title = "매수 판단 리포트" if is_advisory else f"{mode_label} 고객 상담 리포트"
    prologue_copy = (
        "이 리포트는 가격 상승·하락을 단정하지 않습니다. 완료된 공식 거래, 현재 공개 호가, 위치와 현장 조건, 시장 활동을 분리해 비교하고 확인되지 않은 조건은 결론의 제한으로 남깁니다."
        if is_advisory
        else "최근 실제 계약 사례와 지금 나와 있는 매물, 생활권과 현장에서 확인할 장점을 한 번에 살펴봅니다. 고객의 조건에 잘 맞는 이유부터 방문 때 살펴볼 포인트와 상담 순서까지 정리했습니다."
    )
    decision_status = "현재 판단 · 협상 조건 확인" if is_advisory else "한눈에 설명 · 가치 확인"
    subject_eyebrow = "SUBJECT & DECISION FRAME" if is_advisory else "SUBJECT & VALUE FRAME"
    subject_title = "무엇을 사고, 어떤 조건에서 결정할 것인가" if is_advisory else f"{target_name}의 가치를 무엇으로 확인할까요?"
    ladder_title = "가격을 이 범위로 보는 이유와 조건" if is_advisory else "현장에서 확인할 세 가지 가치"
    price_title = "검토 조건은 거래 근거와 회수 조건에서 어디에 있을까요?" if is_advisory else "검토 조건은 현재 거래 근거에서 어디에 있을까요?"
    analysis_card_title = "판단 포인트" if is_advisory else "상담 포인트"
    final_action_title = "권장 행동" if is_advisory else "다음 행동"
    final_action_body = (
        "가격 제안은 확인된 거래와 계산 근거 안에서 정하고, 세대 상태·권리·자금 조건을 확인한 뒤 최종 범위를 조율합니다."
        if is_advisory
        else "확인된 장점을 먼저 살펴보고, 현장에서 물건의 실제 가치를 확인한 뒤 고객에게 맞는 조건을 상담해 보세요."
    )

    pages = []
    pages.append(f'''<section class="sheet page cover dark">{badge()}<div class="cover-main"><div class="eyebrow">{esc(brand_name)} · {cover_eyebrow}</div><h1 class="cover-title">{esc(target_name)}<br>{esc(cover_report_title)}</h1><p class="cover-sub">{esc(role_copy["cover"])}</p><div class="cover-facts"><span>{esc(target.get("descriptor", ""))}</span><span>{esc(price_text)}</span><span>{esc(horizon_text)}</span><span>기준일 {esc(data.get("basis_date", ""))}</span></div></div>{cover_visual}{page_footer(brand_name, 1)}</section>''')

    scope_items = [
        ("1", "대상과 고객 질문", f'{target_name} · {customer.get("role", "고객")} 관점'),
        ("2", "실제 계약 사례", f'같은 비교 범위에서 계약된 사례 {len(official_rows)}건 확인'),
        ("3", "현재 선택지", f'지금 나와 있는 매물 {len(listings)}건과 가격 비교' if listings else "방문 상담 때 최신 매물을 함께 확인"),
        ("4", "가격 위치", f'{price_text} 조건을 실거래·호가와 비교'),
        ("5", "생활권과 현장 가치", "검증된 도보 10분 경로와 현장 체크"),
        ("6", "시장 활동과 다음 행동", "거래 방향을 협상·결정 흐름으로 연결" if is_advisory else "거래 방향을 상담 흐름으로 연결"),
    ]
    ...
    pages.append(f'''<section class="sheet page">{badge()}<div class="eyebrow">PROLOGUE</div><h1 class="h1 question-title">{esc(customer.get("question", "어떤 선택이 좋을까요?"))}</h1><p class="prologue-copy">{esc(prologue_copy)}</p><div class="analysis-list">{scope_html}</div><div class="decision-band"><div><span class="status">{esc(decision_status)}</span><p>{esc(role_copy["fit"])}</p></div><strong>{esc(role_copy["cta"])}</strong></div>{page_footer(brand_name, 2)}</section>''')
    ...
    if is_advisory:
        advisory_steps = [
            ("1차 제안", _decision_amount(decision_context, "first_offer_krw"), "사용자 입력 또는 검증 가능한 비교·계산 근거가 있을 때만 제시합니다."),
            ("조건부 상단", _decision_amount(decision_context, "conditional_ceiling_krw"), "층·전망·수리·권리와 자금 조건이 확인될 때 적용합니다."),
            ("보류 기준", _decision_amount(decision_context, "hold_above_krw"), "근거가 없거나 미확인 위험이 남으면 즉시 계약하지 않고 재산정합니다."),
        ]
        check_html = "".join(
            f'<div class="step advisory-step"><span class="pill">{esc(label)}</span><b>{esc(value)}</b><p>{esc(body)}</p></div>'
            for label, value, body in advisory_steps
        )
        break_even = _decision_amount(decision_context, "break_even_price_krw")
        frame_title = "보유·회수 조건"
        frame_body = "취득부터 보유와 매도까지 들어가는 비용을 함께 계산해야, 어느 가격까지 검토할지 구체적으로 설명할 수 있습니다."
        frame_callout = f"검증된 손익분기 가격 · {break_even}"
        frame_note = "세율·보수·금융비용은 개인 상황과 계약 시점에 따라 달라집니다."
        perspective_title = "매수자 관점의 핵심"
        perspective_callout = "공식 거래와 공개 호가를 구분하고, 가격 차이를 설명할 현장 조건이 확인될 때만 협상 상단을 적용합니다."
    else:
        ...
        frame_title = "실거주 관점"
        frame_body = "직접 이용하는 동안 가격뿐 아니라 채광, 전망, 내부 상태, 관리 여건이 생활 만족도와 향후 거래 경쟁력에 함께 작용합니다."
        frame_callout = "현재 조건 범위에서 대상을 우선 검토하고, 현장에서 추가 가치를 확인합니다."
        frame_note = "취득·중개·등기·금융비용은 개인 상황에 따라 달라지므로 계약 상담 단계에서 별도로 확인합니다."
        perspective_title = "고객 관점의 핵심"
        perspective_callout = "확인된 숫자는 비교 기준으로 사용하고, 실제 경쟁력은 현장에서 확인되는 가치 요소와 함께 설명합니다."
    pages.append(f'''<section class="sheet page">{badge()}<div class="eyebrow">{esc(subject_eyebrow)}</div><h2 class="h2">{esc(subject_title)}</h2><div class="grid3">{cards}</div><div class="grid2 value-frame"><div class="card soft"><h3 class="h3">시장 비교 기준</h3><table class="table"><tbody><tr><td>검토 조건</td><td><b>{esc(price_text)}</b></td></tr><tr><td>최근 유효 거래</td><td><b>{esc(fmt_krw(latest_value))}</b></td></tr><tr><td>실거래 범위</td><td><b>{esc((fmt_krw(min(official_values)) + " ~ " + fmt_krw(max(official_values))) if official_values else "자료 보완")}</b></td></tr><tr><td>공개 호가 범위</td><td><b>{esc((fmt_krw(min(listing_prices)) + " ~ " + fmt_krw(max(listing_prices))) if listing_prices else "현장 상담 시 확인")}</b></td></tr></tbody></table></div><div class="card"><h3 class="h3">{esc(frame_title)}</h3><p>{esc(frame_body)}</p><div class="callout">{esc(frame_callout)}</div><p class="note">{esc(frame_note)}</p></div></div><h3 class="h3 value-heading">{esc(ladder_title)}</h3><div class="ladder">{check_html}</div><div class="perspective-note"><b>{esc(perspective_title)}</b><span>{esc(perspective_callout)}</span></div>{page_footer(brand_name, 3)}</section>''')
    ...
    pages.append(f'''<section class="sheet page">{badge()}<div class="eyebrow">PRICE POSITION</div><h2 class="h2">{esc(price_title)}</h2><div class="grid3 compact-metrics">{cards}</div><div class="chart dominant-chart">{official_visual}</div><div class="grid2 evidence-copy"><div class="card"><h3 class="h3">가격을 읽는 기준</h3><p>{esc(official_section.get("body", "공식 거래와 고객 조건을 같은 비교범위에서 확인합니다."))}</p></div><div class="card orange"><h3 class="h3">{esc(analysis_card_title)}</h3><p>{esc(role_copy["fit"])}</p></div></div>{source_note_official}{page_footer(brand_name, 5)}</section>''')
    ...
    transaction_callout = (
        "같은 면적과 비슷한 조건에서도 세대 상태와 계약 시점에 따라 가격 차이가 생깁니다. 미확인 차이를 임의의 프리미엄으로 보지 말고 협상 조건으로 남깁니다."
        if is_advisory
        else "같은 면적과 비슷한 조건에서도 세대 상태와 계약 시점에 따라 차이가 생깁니다. 현장에서 대상 물건의 장점을 확인하면 가격 경쟁력을 더 구체적으로 설명할 수 있습니다."
    )
    third_insight_label = "NEGOTIATE" if is_advisory else "CONSULT"
    third_insight_title = "협상 조건" if is_advisory else "조건 상담"
    third_insight_body = "잔금·인도·관리·권리 조건을 확인한 뒤 검증된 상단 범위 안에서 결정합니다." if is_advisory else "잔금·인도·관리·권리 조건을 함께 정리해 고객 상담으로 연결합니다."
    transaction_history = _transaction_history_svg(official_rows, mode, number(price_context))
    pages.append(f'''<section class="sheet page">{badge()}<div class="eyebrow">OFFICIAL TRANSACTIONS</div><h2 class="h2">실제 계약 사례를 조건별로 확인합니다</h2><div class="grid2 transaction-head"><div class="card soft"><div class="metric">{len(official_rows)}건</div><div class="metric-label">선택 범위의 유효 거래</div></div><div class="card"><h3 class="h3">거래 범위 해석</h3><p>{esc(range_copy)}</p></div></div><div class="chart transaction-history-chart">{transaction_history}</div><div class="card transaction-card"><table class="table transaction-table"><thead><tr><th>계약일</th><th>동·구분</th><th>층</th><th>거래조건</th></tr></thead><tbody>{table_rows}</tbody></table></div><div class="callout navy">{esc(transaction_callout)}</div><div class="grid3 trade-insights"><div class="card"><span class="pill">COMPARE</span><h3 class="h3">같은 범위 비교</h3><p>동일 유형·면적·기간의 계약 사례를 기준으로 가격 차이를 설명합니다.</p></div><div class="card"><span class="pill">VALUE</span><h3 class="h3">현장 가치 확인</h3><p>층·향·채광·전망·수리 상태가 실제 경쟁력을 만드는지 확인합니다.</p></div><div class="card orange"><span class="pill">{third_insight_label}</span><h3 class="h3">{esc(third_insight_title)}</h3><p>{esc(third_insight_body)}</p></div></div>{source_note_official}{page_footer(brand_name, 6)}</section>''')
    ...
    competition_card_title = "계약가 기준 경쟁력" if is_advisory else "고객에게 보여줄 가치"
    competition_card_body = (
        "공개 호가보다 낮더라도 완료된 최근 거래보다 높을 수 있습니다. 층·전망·수리 상태가 가격 차이를 설명하는지 확인한 뒤 협상합니다."
        if is_advisory
        else "대상 물건의 위치·상태·관리·이용 장점을 비교하면 공개 호가 안에서의 경쟁력을 구체적으로 설명할 수 있습니다."
    )
    competition_band_title = "공개 호가는 실제 계약가격과 다를 수 있습니다." if is_advisory else "현재 선택지와 비교하면 매물의 장점이 더 선명해집니다."
    competition_band_body = (
        "공개 호가와 완료 거래의 차이를 협상 근거로 사용하되, 표본의 중복·변동과 세대 조건을 현장에서 재확인합니다."
        if is_advisory
        else "대상 물건의 장점과 실제 거래 가능 조건을 현장에서 확인하면 현재 선택지 안에서의 경쟁력을 더 선명하게 설명할 수 있습니다."
    )
    current_choice_body = (
        str(listing_section.get("body", "공개 호가의 범위와 실제 거래 가능 여부를 확인합니다."))
        if is_advisory
        else "지금 확인한 매물은 조회 시점의 선택지입니다. 실제 거래 가능 여부와 세대별 장점은 방문 상담 때 함께 확인하겠습니다."
    )
    pages.append(f'''<section class="sheet page">{badge()}<div class="eyebrow">CURRENT COMPETITION</div><h2 class="h2">현재 공개매물에서 대상의 경쟁력을 확인합니다</h2><div class="grid3 compact-metrics"><div class="card metric-card"><div class="metric">{(str(len(listings)) + "건") if listings else "상담 시 확인"}</div><div class="metric-label">확인한 공개매물</div></div><div class="card metric-card"><div class="metric range-metric">{esc(listing_range)}</div><div class="metric-label">현재 나온 가격 범위</div></div><div class="card metric-card orange"><div class="metric orange">{esc(price_text)}</div><div class="metric-label">고객 검토 조건</div></div></div><div class="chart dominant-chart listing-chart">{listing_visual}</div><div class="grid2 evidence-copy"><div class="card"><h3 class="h3">현재 선택지</h3><p>{esc(current_choice_body)}</p></div><div class="card orange"><h3 class="h3">{esc(competition_card_title)}</h3><p>{esc(competition_card_body)}</p></div></div><div class="competition-band"><b>{esc(competition_band_title)}</b><span>{esc(competition_band_body)}</span></div>{source_note_listing}{page_footer(brand_name, 8)}</section>''')
