...
def render_golden_v3(
    ...
    *,
    ...
    if not listings:
        listing_visual = '''<div class="visual-empty listing-empty"><span class="empty-kicker">EVIDENCE LIMITATION</span><b>현재 공개매물 표본 자료 보완</b><p>자료가 없다는 사실을 임의의 호가나 가격 추정으로 대체하지 않습니다.</p><div class="empty-steps"><span><i>1</i>공개 표본 확인</span><span><i>2</i>중복·변동 정리</span><span><i>3</i>실제 거래 가능 여부 재확인</span></div></div>'''
    ...
    prologue_copy = (
        ...
        if is_advisory
        else "이 리포트는 공식 거래, 현재 공개매물, 대상의 위치와 현장 가치, 시장 활동을 분리해 비교합니다. 확인된 근거를 바탕으로 고객이 계속 관심을 갖고 현장에서 가치를 확인할 수 있도록 상담 포인트를 정리했습니다."
    )
    ...
    ladder_title = "근거가 있을 때만 쓰는 협상 가격 사다리" if is_advisory else "현장에서 확인할 세 가지 가치"
    ...
    final_action_body = (
        "사용자가 제공했거나 계산 기록으로 검증된 협상 범위만 사용합니다. 현장·권리·자금 조건이 확인되기 전에는 미확인 위험을 가격에 반영하지 않습니다."
        if is_advisory
        else "확인된 시장 근거를 바탕으로 현장에서 물건의 실제 가치를 점검하고, 고객에게 맞는 조건을 상담합니다."
    )
    ...
    if is_advisory:
        ...
        frame_body = "거래비용, 금융비용, 수리비, 보유세와 처분비용을 포함한 계산 기록이 있어야 회수 조건을 숫자로 제시할 수 있습니다."
    ...
    if is_advisory:
        basis = str(decision_context.get("recommendation_basis") or "협상 숫자 계산 근거 추가 필요")
        ...
        recommendation_basis_html = f'<div class="advisory-basis"><b>협상 숫자 근거</b><span>{esc(basis)}</span><small>{esc(basis_date)} · {esc(basis_sources)}</small></div>'
    pages.append(f'''<section class="sheet page final-page dark">{badge()}<div class="eyebrow final-eyebrow">FINAL DECISION & NEXT ACTION</div><div class="final-decision">이번 분석을 정리하면,<br><em>{esc(role_copy["headline"])}</em></div><div class="grid2 final-grid"><div class="card darkcard"><h3 class="h3 light">{esc(final_basis_label)}</h3><table class="table dark-table"><tbody><tr><td>검토 조건</td><td><b>{esc(price_text)}</b></td></tr><tr><td>유효 거래</td><td><b>{len(official_rows)}건</b></td></tr><tr><td>공개매물 표본</td><td><b>{(str(len(listings)) + "건") if listings else "자료 보완"}</b></td></tr><tr><td>계획</td><td><b>{esc(horizon_text)}</b></td></tr></tbody></table><p class="dark-note">금리·지역 시장 자료가 수집된 경우 날짜와 원출처를 표시하며, 예측이 아닌 자금·심리 환경으로 분리해 해석합니다.</p></div><div class="card"><h3 class="h3">핵심 판단</h3><p>{esc(role_copy["fit"])}</p><h3 class="h3 next-title">확인할 사항</h3><ul class="checklist">{final_check_html}</ul></div></div><div class="card orange final-action"><h3 class="h3">{esc(final_action_title)}</h3><p>{esc(final_action_body)}</p><div class="callout orange-callout">{esc(role_copy["cta"])}</div></div>{recommendation_basis_html}<div class="source-block"><ul class="sources">{sources_html}</ul></div><div class="verification">엔진 {ENGINE_ID} {ENGINE_VERSION} · 품질 {QUALITY_PROFILE} · 기준일 {esc(data.get("basis_date", ""))} · {esc(release_status)} · 검증ID {esc(verification_id)}</div><p class="closing-disclaimer">{esc(data.get("disclaimer", ""))}</p>{page_footer(brand_name, 9)}</section>''')
