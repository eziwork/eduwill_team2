...
$layoutPath = Join-Path $resolvedReportRoot "layout-inspection.json"
$qualityPath = Join-Path $resolvedReportRoot "visual-quality.json"
...
if (-not $SkipPdf) {
    Invoke-Checked -Executable $NodeExe -Arguments @((Join-Path $scriptDir "inspect_layout.mjs"), "--input", $htmlPath, "--output", $layoutPath)
    ...
    $qualityArguments = @(
        (Join-Path $scriptDir "visual_quality_gate.py"),
        "--pdf", $pdfPath,
        "--html", $htmlPath,
        "--layout", $layoutPath,
        "--request", $requestPath,
    ...
    if (-not [string]::IsNullOrWhiteSpace($ReferencePdf)) {
        $qualityArguments += @("--reference-pdf", (Resolve-Path -LiteralPath $ReferencePdf).Path)
    }
    Invoke-Checked -Executable $PythonExe -Arguments $qualityArguments
}
...
[pscustomobject][ordered]@{
    ...
    layout = if ($SkipPdf) { $null } else { $layoutPath }
    visual_quality = if ($SkipPdf) { $null } else { $qualityPath }
    reference_pdf = if ([string]::IsNullOrWhiteSpace($ReferencePdf)) { $null } else { (Resolve-Path -LiteralPath $ReferencePdf).Path }
