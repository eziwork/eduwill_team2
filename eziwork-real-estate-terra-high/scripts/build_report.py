...
from golden_v3_builder import ENGINE_ID, ENGINE_VERSION, PAGE_COUNT, render_golden_v3
...
def validate_request(data: dict[str, Any]) -> list[str]:
    ...
    is_checkpoint = data.get("report_type") == "checkpoint"
    required = {
    ...
    if is_checkpoint:
        ...
        if data.get("customer_type") not in {"BUY", "SELL"}:
            errors.append("customer_type must be BUY or SELL for checkpoint reports")
        try:
            view = selected_checkpoint_view(data)
            errors.extend(validate_page_plan(data, view))
        except Exception as exc:
            errors.append(f"checkpoint request cannot be resolved: {exc}")
    else:
        engine = data.get("report_engine") if isinstance(data.get("report_engine"), dict) else {}
        if str(engine.get("id", "")) != ENGINE_ID:
            errors.append(f"standard reports must use {ENGINE_ID}")
        if str(engine.get("version", "")) != ENGINE_VERSION:
            errors.append(f"standard reports must use engine version {ENGINE_VERSION}")
        if int(engine.get("page_count", 0) or 0) != PAGE_COUNT:
            errors.append(f"standard reports must declare {PAGE_COUNT} pages")
        if data.get("report_profile") != "EXTENDED_9":
            errors.append("legacy compact profile is disabled; report_profile must be EXTENDED_9")
    ...
    for index, section in enumerate(data.get("sections", []) if not is_checkpoint else [], start=1):
    ...
    if data.get("evidence_mode") == "actual" and not is_checkpoint:
    ...
    audit["warnings"] = list(dict.fromkeys(migration_warnings + audit.get("warnings", [])))
...
def line_chart(spec: dict[str, Any]) -> str:
    ...
    colors = ["#0a67ff", "#f37021", "#1d875f", "#7b61c9"]
    shapes = []
    legends = []
...
def generic_hero(mode: str) -> str:
    if mode == "land_lease":
        ...
        <svg viewBox="0 0 1200 700" role="img" aria-label="토지 위치 일러스트">
          ...
          <path d="M370 150l420-35 130 255-400 55z" fill="#0a67ff" opacity=".75" stroke="#fff" stroke-width="10"/>
    ...
    if mode == "commercial_lease":
        ...
        <svg viewBox="0 0 1200 700" role="img" aria-label="상가 건물 일러스트">
          ...
          <rect x="225" y="330" width="270" height="240" fill="#0a67ff"/><rect x="525" y="330" width="210" height="240" fill="#233c72"/><rect x="765" y="330" width="210" height="240" fill="#8fa4c4"/>
...
def render_report(data: dict[str, Any]) -> str:
    if data.get("report_type") != "checkpoint":
        return render_golden_v3(
            data,
            render_visual=render_visual,
            image_loader=image_data_uri,
            source_line_renderer=source_line,
            demo_badge_renderer=demo_badge,
        )
    base_dir = Path(data["_base_dir"])
    mode_label, _ = MODES[data["mode"]]
    brand = data["brand"]
    brand_name = str(brand["name"])
    color = str(brand.get("color", "#0a67ff"))
    ...
    pages.append(
        ...
        <section class="sheet closing">
          ...
          <div class="closing-source">기준일 {esc(data['basis_date'])} · {esc(release_status)} · 검증ID {esc(verification_id)}</div>
...
<meta name="evidence-audit-sha256" content="{esc(audit.get('evidence_fingerprint', ''))}">
<meta name="visible-payload-sha256" content="{esc(audit.get('visible_payload_fingerprint', ''))}">
<meta name="asset-manifest-sha256" content="{esc(audit.get('asset_manifest_fingerprint', ''))}">
<meta name="combined-release-sha256" content="{esc(audit.get('combined_release_fingerprint', ''))}">
<meta name="derived-release-status" content="{esc(release_status)}">
<meta name="report-type" content="{esc(data.get('report_type', 'standard'))}">
<meta name="communication-mode" content="{esc(data.get('communication_mode', 'CUSTOMER_SALES'))}">
<meta name="report-profile" content="{esc(data.get('report_profile', 'COMPACT_6'))}">
<meta name="conversion-goal" content="{esc(data.get('conversion_goal', 'SITE_VISIT_CONSULTATION'))}">
<meta name="customer-type" content="{esc(data.get('customer_type', customer.get('role', '')))}">
<meta name="evidence-mode" content="{esc(data.get('evidence_mode', ''))}">
<title>{esc(target['name'])} · {esc(mode_label)} 고객 브리핑</title>
<style>
:root{{--blue:{color};--blue-dark:#082f58;--ink:#17191d;--muted:#6e7580;--line:#d8dde5;--soft:#f4f7fb;--accent:#f37021;--green:#1d875f}}
...
def main() -> int:
    ...
    try:
        ...
        print(f"ERROR: request validation runtime failure: {exc}")
        return 3
    ...
    try:
        ...
        print(f"ERROR: report build runtime failure: {exc}")
        return 3
